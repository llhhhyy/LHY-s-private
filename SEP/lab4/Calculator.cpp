...
ManhattanDistanceCalculator::calculateDistance(const TreeNode &nodeA,
  ...
  // TODO
  return abs(nodeA.getCoordinates()[0]-nodeB.getCoordinates()[0])+abs(nodeA.getCoordinates()[1]-nodeB.getCoordinates()[1]);
}
...
double ManhattanDistanceCalculator::verticalDistance(TreeNode &root,
  ...
  // TODO
    double verd=0;
    verd=abs(root.getCoordinates()[dim]-target.getCoordinates()[dim]);
    return verd;
}
...
EuclideanDistanceCalculator::calculateDistance(const TreeNode &nodeA,
  ...
  // TODO
  double d=0;
  d=sqrt((nodeA.getCoordinates()[0]-nodeB.getCoordinates()[0])*(nodeA.getCoordinates()[0]-nodeB.getCoordinates()[0])+
          (nodeA.getCoordinates()[1]-nodeB.getCoordinates()[1])*(nodeA.getCoordinates()[1]-nodeB.getCoordinates()[1]));
  return d;
}
...
double EuclideanDistanceCalculator::verticalDistance(TreeNode &root,
  ...
  // TODO
  double verd=0;
  verd=root.getCoordinates()[dim]-target.getCoordinates()[dim];
  return verd>0?verd:-verd;
}
